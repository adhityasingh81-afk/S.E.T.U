 ...
 */
export function simulateRippleEffect(affectedNodeId, severityPct = 40, durationDays = 45, eventType = "Supplier Disruption") {
  const affectedNode = NODES.find(n => n.id === affectedNodeId) || NODES[0];
  const severityMultiplier = severityPct / 100;
  ...
  // Factories dependent on this node suffer proportional drop cushioned by their initial inventory buffer
  let factoryDropPct = 0;
  let affectedFactories = [];
  
  if (severityPct > 0) {
    if (affectedNode.type === "supplier") {
      // Find all assembly plants that depend on this specific supplier
  ...
  // Daily revenue of AURA is ₹1.23 Cr; with disruption severity and penalty clauses:
  const dailyBaseLossCr = (COMPANY_PROFILE.annualRevenueCr / 365) * severityMultiplier * 0.65;
  const penaltySurchargesCr = (durationDays * 0.08) * severityMultiplier;
  const totalRevenueAtRiskCr = severityPct > 0 ? Math.round((dailyBaseLossCr * Math.min(durationDays, 25) + penaltySurchargesCr) * 10) / 10 : 0.0;

  // Step 6: Recovery velocity comparison
  const unassistedRecoveryDays = severityPct > 0 ? Math.round(durationDays * 0.42 + 8) : 0;
  ...
  // Step 7: Node-by-node status mapping for graph & map visualization
  const simulatedNodes = NODES.map(node => {
    ...
    if (severityPct > 0) {
      if (node.id === affectedNodeId) {
        ...
        impactNote = `Direct fracture: ${severityPct}% capacity curtailed for ${durationDays} days.`;
  ...
  const propagationTimeline = [
    ...
    },
    {
      ...
      description: `Unmitigated disruption results in extensive contractual penalties and unfulfilled orders.`,
