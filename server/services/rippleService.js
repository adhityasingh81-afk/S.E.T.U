...
export function simulateRipple(
  ...
  let effectiveSeverityPct = Number(severityPct);
  const duration = Number(durationDays);

  if (fractureType === 'blackout') {
    effectiveSeverityPct = 100;
  } else if (fractureType === 'cost') {
    effectiveSeverityPct = Math.round(effectiveSeverityPct * 0.75);
  } else if (fractureType === 'quality') {
    effectiveSeverityPct = Math.round(effectiveSeverityPct * 0.85);
  } else if (fractureType === 'lead-time') {
    effectiveSeverityPct = Math.round(effectiveSeverityPct * 0.9);
  }

  const severityMultiplier = effectiveSeverityPct / 100;

  // 1. Criticality Amplifier
  ...
  const containmentSavingsMap = {
    'activate-backup': 0.42,
    'reserve-buffer': 0.24,
    'prioritize-sla': 0.18,
    'expedite-freight': 0.16,
  };

  let totalContainmentReduction = 0;
  if (Array.isArray(activeContainmentIds)) {
    activeContainmentIds.forEach(id => {
      if (containmentSavingsMap[id]) totalContainmentReduction += containmentSavingsMap[id];
    });
  }
  ...
  const penaltySurchargesCr = (duration * 0.08) * severityMultiplier * (amplificationMultiplier * 0.4);
  const uncontainedTotalRiskCr = effectiveSeverityPct > 0 ? Math.round((dailyBaseLossCr * Math.min(duration, 25) + penaltySurchargesCr) * 10) / 10 : 0.0;
  ...
  const unassistedRecoveryDays = effectiveSeverityPct > 0 ? Math.round(duration * 0.42 + 8) : 0;
  ...
  const cascadeNodes = [
    ...
    },
    {
      ...
      description: `Unmitigated network shock produces ₹${totalRevenueAtRiskCr} Cr cumulative exposure across ${duration} days.`,
  ...
  const simulatedNodes = NODES.map(node => {
    ...
    if (effectiveSeverityPct > 0) {
      if (node.id === affectedNodeId) {
        ...
        impactNote = `Direct fracture: ${effectiveSeverityPct}% ${fractureType} curtailed for ${duration} days.`;
  ...
  return {
    ...
    rawSeverityPct: Number(severityPct),
    ...
    metrics: {
      ...
      capitalSavedCr,
      dailyLossRateCr: (totalRevenueAtRiskCr / Math.min(duration, 25)).toFixed(2),
